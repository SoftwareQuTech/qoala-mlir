 ...
 * instantiated by the MLIR templaces, since it is not a complete definition:
 ...
 * and once in the libMLIRLir.a, since Lir.cpp uses the .h.inc with the  #define GET_TYPEDEF_CLASSES macro).
 * SOLUTION: insert a "shell" declaration here, so the instantiation of the templates work correctly. When linking
 * (and knowing that the C linker is super simple), the "expanded" definition fo this class (which contains methods and stuff)
 * will link with no problems, since the dispacther will find the right mangled method name within the execution binaries.
 */
