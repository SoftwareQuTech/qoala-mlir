...
namespace qoala::iqoala {
    ...
    raw_ostream &operator<<(raw_ostream &os, const helpers::PrintInterface &printable) {
        printable.print(os);
        return os;
    }
    ...
    void iQoalaProgram::addRoutine(QuantumRoutine &routine) {
        if (auto localRoutine = dyn_cast<LocalQuantumRoutine>(&routine)) {
            this->netQASMSection.addRoutine(*localRoutine);
            return;
        }
        if (auto remoteRoutine = dyn_cast<RequestQuantumRoutine>(&routine)) {
            this->requestSection.addRoutine(*remoteRoutine);
            return;
        }
    }
}
