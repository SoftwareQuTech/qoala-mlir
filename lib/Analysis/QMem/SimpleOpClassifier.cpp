...
namespace qoala::analysis::functionize {
    ...
    static bool qMemOpCanBeSimplyFunctionized(const Operation &op) {
        ...
        return llvm::isa<dialects::qmem::CnotOp, dialects::qmem::CzOp, dialects::qmem::EprsMeasureOp,
                         dialects::qmem::EprsOp, dialects::qmem::HadamardOp, dialects::qmem::XOp, dialects::qmem::YOp, dialects::qmem::ZOp,dialects::qmem::InitOp,
                          ...
                          * This allows us to place the call to the builtin angle conversion function before
                         functionizing dialects::qmem::RotateXOp, dialects::qmem::RotateYOp, dialects::qmem::RotateZOp,
                         dialects::qmem::CrotXOp
                          */
                         /* Instead, we need to functionize the "integer" version of the rotations */
                         dialects::qmem::RotateXIntOp, dialects::qmem::RotateYIntOp, dialects::qmem::RotateZIntOp,
        ...
        for (Operation &op : mainFunction.getOps()) {
            if (qMemOpCanBeSimplyFunctionized(op)) {
                ...
                if (auto initOp = llvm::dyn_cast<dialects::qmem::InitOp>(op)) {
                    assert(declaredQubits.contains(initOp.getQ()) &&
                           "Simple functionize: trying to group an init operation "
                ...
                if (auto eprsOp = llvm::dyn_cast<dialects::qmem::EprsOp>(op)) {
                    assert(declaredQubits.contains(eprsOp.getQ()) &&
                           "Simple functionize: trying to group an eprs operation "
                           "whose qubit has not been declared (qalloc)");
                    QuantumOpsGroupTy currentOpsGroup;
                ...
                if (auto eprsOp = llvm::dyn_cast<dialects::qmem::EprsMeasureOp>(op)) {
                    assert(declaredQubits.contains(eprsOp.getQ()) &&
                           "Simple functionize: trying to group an eprs_measure operation "
                           "whose qubit has not been declared (qalloc)");
                    QuantumOpsGroupTy currentOpsGroup;
                    currentOpsGroup.push_back(declaredQubits[eprsOp.getQ()].getOperation());
                    currentOpsGroup.push_back(eprsOp.getOperation());
