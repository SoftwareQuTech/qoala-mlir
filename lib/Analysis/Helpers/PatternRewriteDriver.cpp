...
namespace mlir::port {
    ...
    struct ExpensiveChecks : public RewriterBase::ForwardingListener {
        ...
        void computeFingerPrints(Operation *topLevel) {
            ...
            topLevel->walk([&](Operation *op) { fingerprints.try_emplace(op, op, /*includeNested=*/false); });
        }
    ...
    protected:
        /// Invalidate the fingerprint of the given op, i.e., remove it from the map.
    ...
    bool Worklist::empty() const {
        // Skip all nullptr.
        return !llvm::any_of(list, [](Operation *op) { return static_cast<bool>(op); });
    }
    ...
    public:
        RandomizedWorklist(): Worklist() { generator.seed(MLIR_GREEDY_REWRITE_RANDOMIZER_SEED); }
    ...
    class GreedyPatternRewriteDriver : public PatternRewriter, public RewriterBase::Listener {
    protected:
        explicit GreedyPatternRewriteDriver(MLIRContext *ctx, const FrozenRewritePatternSet &patterns,
...
GreedyPatternRewriteDriver::GreedyPatternRewriteDriver(MLIRContext *ctx, const FrozenRewritePatternSet &patterns,
                                                       const GreedyRewriteConfig &config):
...
bool GreedyPatternRewriteDriver::processWorklist() {
    ...
    while (!worklist.empty() &&
           (numRewrites < config.getMaxNumRewrites() || config.getMaxNumRewrites() == GreedyRewriteConfig::kNoLimit)) {
        auto *op = worklist.pop();

        LLVM_DEBUG({
            logger.getOStream() << "\n";
            logger.startLine() << logLineComment;
            logger.startLine() << "Processing operation : '" << op->getName() << "'(" << op << ") {\n";
            logger.indent();

            // If the operation has no regions, just print it here.
            if (op->getNumRegions() == 0) {
                op->print(logger.startLine(), OpPrintingFlags().printGenericOpForm().elideLargeElementsAttrs());
                logger.getOStream() << "\n\n";
            }
        });
        ...
        if (config.isFoldingEnabled() && !op->hasTrait<OpTrait::ConstantLike>()) {
            ...
            if (succeeded(op->fold(foldResults))) {
                ...
                assert(foldResults.size() == op->getNumResults() && "folder produced incorrect number of results");
                OpBuilder::InsertionGuard g(*this);
                setInsertionPoint(op);
                SmallVector<Value> replacements;
                bool materializationSucceeded = true;
                for (auto [ofr, resultType] : llvm::zip_equal(foldResults, op->getResultTypes())) {
                    if (auto value = dyn_cast<Value>(ofr)) {
                        assert(value.getType() == resultType && "folder produced value of incorrect type");
                        replacements.push_back(value);
                        continue;
                    }
                    // Materialize Attributes as SSA values.
                    Operation *constOp = op->getDialect()->materializeConstant(*this, cast<Attribute>(ofr), resultType,
                                                                               op->getLoc());
                    ...
                    if (!constOp) {
                        ...
                        for (Value replacement : replacements) {
                            assert(replacement.use_empty() && "folder reused existing op for one result but constant "
        ...
        auto canApplyCallback = [&](const Pattern &pattern) {
            LLVM_DEBUG({
                logger.getOStream() << "\n";
                logger.startLine() << "* Pattern " << pattern.getDebugName() << " : '" << op->getName() << " -> (";
        ...
        auto clearFingerprints = llvm::make_scope_exit([&]() { expensiveChecks.clear(); });
#endif // MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS

        LogicalResult matchResult = matcher.matchAndRewrite(op, *this, canApply, onFailure, onSuccess);
...
void GreedyPatternRewriteDriver::addSingleOpToWorklist(Operation *op) {
    if (config.getStrictness() == GreedyRewriteStrictness::AnyOp || strictModeFilteredOps.contains(op)) {
        worklist.push(op);
    }
}
...
void GreedyPatternRewriteDriver::notifyBlockRemoved(Block *block) {
    if (RewriterBase::Listener *listener = config.getListener()) {
        listener->notifyBlockRemoved(block);
    }
}

void GreedyPatternRewriteDriver::notifyOperationInserted(Operation *op) {
    LLVM_DEBUG({ logger.startLine() << "** Insert  : '" << op->getName() << "'(" << op << ")\n"; });
    if (RewriterBase::Listener *listener = config.getListener()) {
...
void GreedyPatternRewriteDriver::notifyOperationModified(Operation *op) {
    LLVM_DEBUG({ logger.startLine() << "** Modified: '" << op->getName() << "'(" << op << ")\n"; });
    if (RewriterBase::Listener *listener = config.getListener()) {
...
void GreedyPatternRewriteDriver::notifyOperationRemoved(Operation *op) {
    LLVM_DEBUG({ logger.startLine() << "** Erase   : '" << op->getName() << "'(" << op << ")\n"; });
    ...
    if (Region *scope = config.getScope(); scope->getParentOp() == op) {
        llvm_unreachable("scope region must not be erased during greedy pattern rewrite");
    }
#endif // NDEBUG
...
void GreedyPatternRewriteDriver::notifyOperationReplaced(Operation *op, ValueRange replacement) {
    LLVM_DEBUG({ logger.startLine() << "** Replace : '" << op->getName() << "'(" << op << ")\n"; });
    if (RewriterBase::Listener *listener = config.getListener()) {
        listener->notifyOperationReplaced(op, replacement);
    }
}

LogicalResult GreedyPatternRewriteDriver::notifyMatchFailure(Location loc,
...
namespace {
    ...
    public:
        explicit RegionPatternRewriteDriver(MLIRContext *ctx, const FrozenRewritePatternSet &patterns,
                                            const port::GreedyRewriteConfig &config, Region &regions);
...
RegionPatternRewriteDriver::RegionPatternRewriteDriver(MLIRContext *ctx, const FrozenRewritePatternSet &patterns,
                                                       const port::GreedyRewriteConfig &config, Region &region):
...
namespace {
    class GreedyPatternRewriteIteration : public tracing::ActionImpl<GreedyPatternRewriteIteration> {
    public:
        ...
        GreedyPatternRewriteIteration(ArrayRef<IRUnit> units, int64_t iteration):
            tracing::ActionImpl<GreedyPatternRewriteIteration>(units), iteration(iteration) { }

        static constexpr StringLiteral tag = "GreedyPatternRewriteIteration";

        void print(raw_ostream &os) const override { os << "GreedyPatternRewriteIteration(" << iteration << ")"; }
...
LogicalResult RegionPatternRewriteDriver::simplify(bool *changed) && {
    ...
    do {
        ...
        if (++iteration > config.getMaxIterations() &&
            config.getMaxIterations() != port::GreedyRewriteConfig::kNoLimit) {
            break;
        }
        ...
        ctx->executeAction<GreedyPatternRewriteIteration>(
                [&] {
                    ...
                    // loops.
                    continueRewrites |= succeeded(eraseUnreachableBlocks(*this, region));

                    continueRewrites |= processWorklist();

                    // After applying patterns, make sure that the CFG of each of the
                    // regions is kept up to date.
                    if (config.getRegionSimplificationLevel() != GreedySimplifyRegionLevel::Disabled) {
...
LogicalResult port::applyPatternsGreedily(Region &region, const FrozenRewritePatternSet &patterns,
    ...
    if (failed(verify(config.getScope()->getParentOp()))) {
        llvm::report_fatal_error("greedy pattern rewriter input IR failed to verify");
    }
#endif // MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS

    // Start the pattern driver.
    RegionPatternRewriteDriver driver(region.getContext(), patterns, config, region);
    LogicalResult converged = std::move(driver).simplify(changed);
    if (failed(converged)) {
        llvm::dbgs() << "The pattern rewrite did not converge after scanning " << config.getMaxIterations() << " times";
    }
    return converged;
}
...
// LogicalResult port::applyOpPatternsGreedily(
