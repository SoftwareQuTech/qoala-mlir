...
// CHECK: b2 { type = QL; predecessors = []; dependencies = [b0, b1]; prev_comm = ; prev_ent = }:
// CHECK-NEXT: tuple<%2; %3> = run_subroutine() : __qoala_wrapper2
// CHECK: b3 { type = QC; predecessors = []; dependencies = []; prev_comm = ; prev_ent = b0}:
...
module {
  ...
  qoalahost.main_func @test_call_request_routines() {
    ...
    ^bb3:
      qoalahost.blk_meta  {block_id = "block_3", dependencies = [], predecessors = [], prev_comm = "", prev_ent = "block_0"}
      %q2 = qoalahost.call @__qoala_wrapper3() : () -> (i32)
    ^bb4:
      qoalahost.blk_meta  {block_id = "block_4", dependencies = [], predecessors = [], prev_comm = "", prev_ent = ""}
      qoalahost.return
  }
}