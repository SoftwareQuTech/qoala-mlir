...
// CHECK: qoalahost.blk_meta {block_id = "block_1", predecessors = ["block_0"]}
        ...
        qoalahost.nop_term
    ^bb2:
// CHECK: qoalahost.blk_meta {block_id = "block_2", predecessors = ["block_0"]}
        %resB = arith.subi %cstA, %cstB : i32
        qoalahost.nop_term
    ^bb3:
// CHECK: qoalahost.blk_meta {block_id = "block_3", predecessors = []}
        %recvA = qoalahost.recv_floats {length = 1 : i32, remote = @Bob} : tensor<1xf32>
    ^bb4:
// CHECK: qoalahost.blk_meta {block_id = "block_4", predecessors = []}
        %q0 = qoalahost.call @__qoala_wrapper0() : () -> i32
    ^bb5:
// CHECK: qoalahost.blk_meta {block_id = "block_5", predecessors = ["block_0", "block_1", "block_2"]}
        %resC = arith.muli %resA, %resB : i32
        %resD = arith.muli %cstC, %resC : i32
        qoalahost.nop_term
    ^bb6:
// CHECK: qoalahost.blk_meta {block_id = "block_6", predecessors = ["block_1", "block_2", "block_5"]}
        %resE = arith.muli %resA, %resD : i32
        %resF = arith.muli %resE, %resB : i32
        qoalahost.nop_term
    ^bb7:
// CHECK: qoalahost.blk_meta {block_id = "block_7", predecessors = ["block_0", "block_3", "block_6"]}
        ...
        qoalahost.nop_term
    ^bb8:
// CHECK: qoalahost.blk_meta {block_id = "block_8", predecessors = ["block_7"]}
        %recvB = qoalahost.recv_ints {length = 1 : i32, remote = @Bob} : tensor<1xi32>
    ^bb9:
// CHECK: qoalahost.blk_meta {block_id = "block_9", predecessors = ["block_4"]}
        %q1 = qoalahost.call @__qoala_wrapper1() : () -> i32
    ^bb10:
// CHECK: qoalahost.blk_meta {block_id = "block_10", predecessors = ["block_4"]}
        %q2 = qoalahost.call @__qoala_wrapper2(%q0) : (i32) -> (i32)
    ^bb11:
// CHECK: qoalahost.blk_meta {block_id = "block_11", predecessors = ["block_6", "block_8"]}
        %sendC = tensor.from_elements %resE : tensor<1xi32>
        qoalahost.send_ints %sendC {remote = @Bob} : tensor<1xi32>
        qoalahost.nop_term
    ^bb12:
// CHECK: qoalahost.blk_meta {block_id = "block_12", predecessors = ["block_10", "block_9"]}
        %m0, %m1 = qoalahost.call @__qoala_wrapper3(%q2, %q1) : (i32, i32) -> (i1, i1)
    ^bb13:
// CHECK: qoalahost.blk_meta {block_id = "block_13", predecessors = ["block_0", "block_11"]}
