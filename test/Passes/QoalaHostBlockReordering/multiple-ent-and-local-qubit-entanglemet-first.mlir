...
module {
  ...
  netqasm.request_routine @entanglement_2() -> i32 {
    %1 = netqasm.qalloc  : i32
    netqasm.eprs %1  {remote = @Charlie}
    netqasm.return %1 : i32
  }
  netqasm.local_routine @meas_ent_2(%1: i32) -> i1 {
    %m1 = netqasm.measure %1 : i1
    netqasm.return %m1 : i1
  }
  qoalahost.main_func @test_reordering_teleport() {
    ...
    ^bb7:
        qoalahost.blk_meta  {block_id = "block_7", deadlines = {}, dependencies = ["block_99", "block_5"], predecessors = [], prev_comm = "", prev_ent = ""}
        ...
        qoalahost.nop_term
    ^bb8:
        qoalahost.blk_meta  {block_id = "block_8", deadlines = {}, dependencies = ["block_99", "block_6"], predecessors = [], prev_comm = "block_7", prev_ent = ""}
        ...
        qoalahost.nop_term
    ^bb9:
        // We enforce a dependence to block_8 which does not exist to make sure it would be optimal to do the second entanglement not as the first block
        qoalahost.blk_meta  {block_id = "block_9", deadlines = {}, dependencies = ["block_1", "block_8"], predecessors = [], prev_comm = "", prev_ent = ""}
        %m2 = qoalahost.call @meas_ent_2(%0) : (i32) -> i1
  }
}
