...
module {
  ...
  // CHECK: netqasm.local_routine @[[WRAPPER0:.*]](%[[ARG0_2:.*]]: i32, %[[ARG1_2:.*]]: i32, %[[ARG2_2:.*]]: i32, %[[ARG3_2:.*]]: i32, %[[ARG4_2:.*]]: i32, %[[ARG5_2:.*]]: i32, %[[ARG6_2:.*]]: i32, %[[ARG7_2:.*]]: i32) -> (i1, i1)
  ...
  // CHECK-NEXT: netqasm.rot_x %[[REG0_0]], %[[ARG0_2]], %[[ARG1_2]]
  // CHECK-NEXT: netqasm.rot_y %[[REG0_0]], %[[ARG2_2]], %[[ARG3_2]]
  // CHECK-NEXT: netqasm.rot_z %[[REG0_0]], %[[ARG4_2]], %[[ARG5_2]]
  // CHECK-NEXT: netqasm.hadamard %[[REG1_0]]
  // CHECK-NEXT: netqasm.cnot %[[REG0_0]], %[[REG1_0]]
  // CHECK-NEXT: netqasm.cz %[[REG0_0]], %[[REG1_0]]
  // CHECK-NEXT: netqasm.crot_x %[[REG0_0]], %[[REG1_0]], %[[ARG6_2]], %[[ARG7_2]]
  ...
  qmem.func @test_local_quantum_program() {
    ...
    // CHECK-NEXT: %[[REG_MAIN0:.*]]:2 = qoalahost.call @__qoala_convert_float_angle(%[[CST_0]]) : (f32) -> (i32, i32)
    %cst_0 = arith.constant 0.0306796152 : f32

    qmem.rot_x %0, %cst_0

    // CHECK: %[[CST_1:.*]] = arith.constant
    // CHECK-NEXT: %[[REG_MAIN1:.*]]:2 = qoalahost.call @__qoala_convert_float_angle(%[[CST_1]]) : (f32) -> (i32, i32)
    %cst_1 = arith.constant 1.050000e+01 : f32
    qmem.rot_y %0, %cst_1

    // CHECK: %[[CST:.*]] = arith.constant
    // CHECK-NEXT: %[[REG_MAIN2:.*]]:2 = qoalahost.call @__qoala_convert_float_angle(%[[CST]]) : (f32) -> (i32, i32)
    ...
    // CHECK-NEXT: %[[REG_MAIN3:.*]]:2 = qoalahost.call @__qoala_convert_float_angle(%[[CST_2]]) : (f32) -> (i32, i32)
    %cst_2 = arith.constant 2.710000e+00 : f32
    // CHECK: %[[REG_MAIN4:.*]]:2 = qoalahost.call @[[WRAPPER0]](%[[REG_MAIN0]]#0, %[[REG_MAIN0]]#1, %[[REG_MAIN1]]#0, %[[REG_MAIN1]]#1, %[[REG_MAIN2]]#0, %[[REG_MAIN2]]#1, %[[REG_MAIN3]]#0, %[[REG_MAIN3]]#1) : (i32, i32, i32, i32, i32, i32, i32, i32) -> (i1, i1)
