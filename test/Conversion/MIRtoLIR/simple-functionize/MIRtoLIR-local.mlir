...
module {
  ...
  // CHECK: netqasm.local_routine @[[WRAPPER1:.*]](%[[ARG0_0:.*]]: i32)
  // CHECK-NEXT: netqasm.init %[[ARG0_0]]
  ...
  // CHECK: netqasm.local_routine @[[WRAPPER3:.*]](%[[ARG3_0:.*]]: i32)
  ...
  // CHECK: netqasm.local_routine @[[WRAPPER8:.*]](%[[ARG0_8:.*]]: i32, %[[ARG1_8:.*]]: i32)
  // CHECK-NEXT: netqasm.cnot %[[ARG0_8]], %[[ARG1_8]]
  // CHECK-NEXT: netqasm.return

  // CHECK: netqasm.local_routine @[[WRAPPER9:.*]](%[[ARG0_9:.*]]: i32, %[[ARG1_9:.*]]: i32)
  ...
  // CHECK: netqasm.local_routine @[[WRAPPER12:.*]](%[[ARG0_12:.*]]: i32) -> i1
  ...
  qmem.func @test_local_quantum_program() {
    ...
    // CHECK: ^[[BLOCK_3:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_3", predecessors = ["block_2"]}
    ...
    // CHECK: ^[[BLOCK_4:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_4", predecessors = ["block_0"]}
    ...
    // CHECK: ^[[BLOCK_5:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_5", predecessors = ["block_0"]}
    // CHECK-NEXT: qoalahost.call @[[WRAPPER5]](%[[REG_MAIN0]]) : (i32) -> ()
    qmem.rot_y %0, %cst_1

    // CHECK: ^[[BLOCK_6:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_6", predecessors = ["block_0"]}
    // CHECK-NEXT: qoalahost.call @[[WRAPPER6]](%[[REG_MAIN0]]) : (i32) -> ()
    qmem.rot_z %0, %cst

    // CHECK: ^[[BLOCK_7:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_7", predecessors = ["block_2"]}
    // CHECK-NEXT: qoalahost.call @[[WRAPPER7]](%[[REG_MAIN1]]) : (i32) -> ()
    qmem.hadamard %1

    // CHECK: ^[[BLOCK_8:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_8", predecessors = ["block_0", "block_2"]}
    ...
    // CHECK: ^[[BLOCK_9:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_9", predecessors = ["block_0", "block_2"]}
    // CHECK-NEXT: qoalahost.call @[[WRAPPER9]](%[[REG_MAIN0]], %[[REG_MAIN1]]) : (i32, i32) -> ()
    qmem.cz %0, %1

    // CHECK: ^[[BLOCK_10:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_10", predecessors = ["block_0", "block_2"]}
    // CHECK-NEXT: qoalahost.call @[[WRAPPER10]](%[[REG_MAIN0]], %[[REG_MAIN1]]) : (i32, i32) -> ()
    qmem.crot_x %0, %1, %cst_2

    // CHECK: ^[[BLOCK_11:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_11", predecessors = ["block_0"]}
    // CHECK-NEXT: %[[REG_MAIN6:.*]] = qoalahost.call @[[WRAPPER11]](%[[REG_MAIN0]]) : (i32) -> i1
    %2 = qmem.measure %0 : i1

    // CHECK: ^[[BLOCK_12:.*]]:
    // CHECK-NEXT: qoalahost.blk_meta {block_id = "block_12", predecessors = ["block_2"]}
    // CHECK-NEXT: %[[REG_MAIN7:.*]] = qoalahost.call @[[WRAPPER12]](%[[REG_MAIN1]]) : (i32) -> i1
    %3 = qmem.measure %1 : i1
    // CHECK: qoalahost.blk_meta {block_id = "block_13", predecessors = []}
    qmem.return
  }
}