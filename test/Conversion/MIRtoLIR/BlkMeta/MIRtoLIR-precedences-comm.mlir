...
module {
    ...
    qmem.func @test_add_cc_block_deps() {
        ...
        // CHECK: qoalahost.blk_meta  {block_id = "block_1", dependencies = [], predecessors = [], prev_comm = "block_0", prev_ent = ""}
        %to_recvf = qmem.recv_floats {length = 2 : i32, remote = @Bob} : tensor<2xf32>

        // CHECK: qoalahost.blk_meta  {block_id = "block_2", dependencies = [], predecessors = [], prev_comm = "block_1", prev_ent = ""}
        %to_recvi = qmem.recv_ints {length = 2 : i32, remote = @Bob} : tensor<2xi32>

        // CHECK: qoalahost.blk_meta  {block_id = "block_3", dependencies = ["block_0"], predecessors = [], prev_comm = "block_2", prev_ent = ""}
