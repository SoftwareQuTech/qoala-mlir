...
namespace qoala::analysis::isolate {
     ...
     * @param rewriter The convertion patter rewriter to create new block terminators as needed.
     */
    ...
    /**
     * Isolate all the nested operations of a given type. The search will start from a given base
     * operation and isolate all the found operations into a single block. Since creating new
     * isolated blocks might create new blocks with no terminator operations, this method
     * will also create new terminator operations as needed.
     * This function also allows passing an extra check function to exclude the isolation of
     * certain nested operations despite they match the types.
     * @tparam FuncOpTy Type of the base function to start exploring for nested operations. This
     *                  type should usually be "qoalahost::MainFuncOp", but can easily be other
     *                  type with a nested region.
     * @tparam OpTy Types of the operations to isolate.
     * @param baseOp The base operation to explore for functions to isolate.
     * @param rewriter A ConversionPatternRewriter object to create new block terminators as needed
     * @param extraCheck An optional extra check function. If this given check returns false, the
     *                   involved operation will not be marked to isolate, even if it is of the
     *                   good type.
     */
